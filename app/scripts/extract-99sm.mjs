...
// 99 Speed Mart parts are named "99 Holdings Prospectus (Part N).pdf".
const files = readdirSync(REPO)
  .filter((f) => /^99 Holdings Prospectus .*\.pdf$/i.test(f))
  .sort();
...
writeFileSync(join(OUT, '99sm-pages.json'), JSON.stringify({ files, pages: allPages }, null, 0));
writeFileSync(
  join(OUT, '99sm-pages.txt'),
  allPages.map((p, i) => `\n===== PAGE ${i + 1} =====\n${p}`).join('\n'),
);
console.log(`Wrote ${allPages.length} pages -> 99sm`);
