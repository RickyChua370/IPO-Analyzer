...
// SRKK AI parts are named "SRKK AI Berhad (IPO ENG) Part N.pdf".
const files = readdirSync(REPO)
  .filter((f) => /^SRKK AI Berhad .*\.pdf$/i.test(f))
  .sort();
...
writeFileSync(join(OUT, 'srkk.json'), JSON.stringify({ files, pages: allPages }, null, 0));
writeFileSync(
  join(OUT, 'srkk.txt'),
  allPages.map((p, i) => `\n===== PAGE ${i + 1} =====\n${p}`).join('\n'),
);
console.log(`Wrote ${allPages.length} pages -> srkk`);
